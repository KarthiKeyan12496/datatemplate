...
const CrmContacts = () => {
    ...
    () => [
      {
        ...
        Cell: (cellProps: any) => {
          return <input type="checkbox" className="contactCheckBox form-check-input" value={cellProps.row.original.id} onChange={() => deleteCheckbox()} />;
        },
        id: '#',
      },
      {
      ...
      },
      {
        ...
        Cell: (cell: any) => (
          <>
            {cell.row.original.tags.map((item: any, key: any) => (<span className="badge bg-primary-subtle text-primary me-1" key={key}>{item}</span>))}
          </>
        ),
      },
      {
        ...
        Cell: (contact: any) => (
          <>
            {handleValidDate(contact.row.original.date)},{" "}
            <small className="text-muted">{handleValidTime(contact.row.original.date)}</small>
          </>
        ),
      },
      {
  ...
  document.title = "Contacts | Skyfire - React Admin & Dashboard Template";
  return (
    <React.Fragment>
